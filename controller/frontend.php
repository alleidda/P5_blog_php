...
class Frontend
{
    ...
    public function contact()
    {
        ...
        if ($decode['success'] == true) {
            ...
            $mail->Port       = SMTP_PORT;
            $mail->setFrom($secure_mail, $user_name);
            $mail->addAddress(SMTP_USER, 'Blog @addielladotexe');
            $mail->addReplyTo($secure_mail, $user_name);
            $mail->isHTML(true);
            $mail->Subject = 'Blog, nouveau message d\'un vitieur';
            // Create a message
            $mail->Body = 'Message du visiteur : ' . nl2br(htmlspecialchars($_POST['message']));
            $mail->send();
    ...
    public function signUp()
    {
        ...
        if ($decode['success'] == true) {
            ...
            $mail->SMTPSecure = SMTP_SECURE;
            $mail->Port       = SMTP_PORT;
            $mail->setFrom(SMTP_USER, 'Blog @addielladotexe');
            $mail->addAddress($_POST['email'], $_POST['firstname']);
            //Content
            $mail->isHTML(true);
            $mail->Subject = 'Activez votre compte';
            $mail->Body = 'Cliquez sur le lien pour <a href=\'' . nl2br(htmlspecialchars(SMTP_ADRESSE))
            ...
            $this->session->set('message', 'Un email a été envoyé à l\'adresse : '
                . nl2br(htmlspecialchars($_POST['email'])) . ' pour valider votre compte ! Si vous ne voyez
                                        pas l\'email, regardez dans vos courriers indésirables.');
            header('location: ' . $_SERVER["HTTP_REFERER"]);
            return;
        }
